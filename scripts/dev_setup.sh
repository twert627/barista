...
install_packages() {
    ...
    case $DISTRIB in
    ubuntu)
        echo "Installing required packages to compile barista on Ubuntu..."
    ...
    debian) 
        ...
        ;;
    arch)
        echo "Installing required packages to compile barista on Arch..."
        ...
        ;;
    darwin)
        echo "Installing required packages to compile barista on MacOSX..."
...
main() {
    echo "=== Linux (bash) barista dev setup script (by twert) ==="
    echo ""
    check_internet
    check_os
    install_packages
    echo "Development environment setup, compile away uwu"
}
main